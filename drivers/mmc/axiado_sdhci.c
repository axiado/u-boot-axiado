...
//#define debug(fmt, args...) do { printf("%s(): ", __func__); printf(fmt, ##args); } while (0)
...
static int axiado_sdhci_set_ios_post(struct sdhci_host *host)
{
	...
	struct axiado_sdhci_plat *plat = dev_get_platdata(dev);
...
U_BOOT_DRIVER(axiado_sdhci_drv) = {
	...
	.ofdata_to_platdata = axiado_sdhci_ofdata_to_platdata,
	.ops = &sdhci_ops,
	.bind = axiado_sdhci_bind,
	.probe = axiado_sdhci_probe,
	.remove = axiado_sdhci_remove,
	.priv_auto_alloc_size = sizeof(struct axiado_sdhci_priv),
	.platdata_auto_alloc_size = sizeof(struct axiado_sdhci_plat),
};
